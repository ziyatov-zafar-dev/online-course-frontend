...
export default function ModuleEdit() {
	...
	const fetchModule = async () => {
		...
		try {
			...
			setForm({
				courseId: module.courseId || courseIdFromParams || "",
			...
			toast.error("Modul ma'lumotlarini yuklashda xatolik")
			console.error(err)
		} finally {
			setFetching(false)
		}
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			toast.error(
				err.response?.data?.message || "Modulni yangilashda xatolik yuz berdi"
			)
			console.error(err)
		} finally {
			setLoading(false)
			...
			<div className='flex items-center gap-4 mb-6'>
				...
				<div>
					...
					<p className='text-sm text-gray-500 dark:text-gray-400'>
						Modul ma'lumotlarini yangilang
					</p>
				</div>
			</div>
				...
				<form onSubmit={handleSubmit} className='space-y-6'>
					...
					<div className='space-y-2'>
						...
						<p className='text-xs text-gray-500 dark:text-gray-400'>
							URL da ishlatiladigan nom. Avtomatik generatsiya qilinadi.
						</p>
					</div>
					...
					<div className='space-y-2'>
						...
						/>
						<p className='text-xs text-gray-500 dark:text-gray-400'>
							Modul kurs ichida qaysi tartibda ko'rinishi
						</p>
					</div>
